...
export class SummaryCommand implements Command {
  ...
  async execute(): Promise<CommandResult> {
    try {
      ...
      logger.error(`프로젝트 요약 실패: ${errorMsg}`);
      return {
        code: ResultCode.FAILURE,
  ...
  private async handleProjectSummary(
    codeIndex: CodeIndex,
    projectId: string,
  ): Promise<CommandResult> {
    const { meta } = codeIndex;

    logger.header('프로젝트 요약');
  ...
  private handleSystemSummary(
    ...
    const matchedFiles = codeIndex.files.filter(
      f => f.path.toLowerCase().includes(lowerName),
    );
    const matchedComponents = codeIndex.components.filter(
      c => c.name.toLowerCase().includes(lowerName) ||
           c.filePath.toLowerCase().includes(lowerName),
    );
    const matchedApis = codeIndex.apis.filter(
      a => a.path.toLowerCase().includes(lowerName) ||
           a.handler.toLowerCase().includes(lowerName) ||
           a.filePath.toLowerCase().includes(lowerName),
    );
    const matchedScreens = codeIndex.screens.filter(
      s => s.name.toLowerCase().includes(lowerName) ||
           s.route.toLowerCase().includes(lowerName) ||
           s.filePath.toLowerCase().includes(lowerName),
    );
    const matchedPolicies = codeIndex.policies.filter(
    ...
    if (totalMatches === 0) {
      ...
      logger.header(`시스템 요약: "${systemName}"`);
   ...
   */
  private extractAvailableSystems(codeIndex: CodeIndex): string[] {
