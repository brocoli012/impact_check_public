...
import { CodeIndex, PolicyInfo } from '../types/index';
 ...
 */
export class PolicyCheckCommand implements Command {
  ...
  async execute(): Promise<CommandResult> {
    try {
      ...
      if (!codeIndex) {
        logger.error('인덱스가 없습니다. 먼저 /impact reindex를 실행하세요.');
        return {
      ...
      logger.error(`정책 영향도 분석 실패: ${errorMsg}`);
      return {
        code: ResultCode.FAILURE,
  ...
  private handleSummary(codeIndex: CodeIndex): CommandResult {
    const policies = codeIndex.policies;

    logger.header('정책 영향도 분석 - 현황 요약');
    ...
    // 카테고리별 그룹핑
    const categoryMap = new Map<string, PolicyInfo[]>();
    ...
    // 카테고리별 출력
    const categoryStats: Record<string, { policyCount: number; fileCount: number }> = {};
  ...
  private async handlePolicyDetail(
    codeIndex: CodeIndex,
    projectId: string,
    policyName: string,
    ...
    // 보강 주석 로드 시도 (폴백 가능)
    let annotationMap: Map<string, AnnotationFile> = new Map();
    try {
  ...
  private async handleChangeImpact(
    codeIndex: CodeIndex,
    _projectId: string,
    changeDesc: string,
  ): Promise<CommandResult> {
    logger.header('정책 변경 영향도 분석');
  ...
  private getRelatedEdges(
    ...
    for (const node of graph.nodes) {
      ...
      if (
        (comp && comp.filePath === policy.filePath) ||
   ...
   * 보강 주석 상세 출력
   */
  private displayAnnotationDetails(annotation: AnnotationFile, policy: PolicyInfo): void {
    // 해당 정책과 관련된 보강 주석의 정책 정보 찾기
    for (const ann of annotation.annotations) {
      if (!ann.policies) continue;
   ...
   * 변경 내용에서 키워드 추출
   */
  private extractKeywords(changeDesc: string): string[] {
    // 공백, 쉼표, 마침표 등으로 분리
   ...
   */
  private searchByKeywords(codeIndex: CodeIndex, keywords: string[]): PolicyInfo[] {
    ...
    for (const keyword of keywords) {
      ...
      for (const comp of codeIndex.components) {
        if (comp.name.toLowerCase().includes(lowerKeyword)) {
          const compPolicies = codeIndex.policies.filter(
            p => p.filePath === comp.filePath && !matchedSet.has(p.id),
          );
      ...
      for (const api of codeIndex.apis) {
        if (api.path.toLowerCase().includes(lowerKeyword)) {
          const apiPolicies = codeIndex.policies.filter(
            p => p.filePath === api.filePath && !matchedSet.has(p.id),
          );
   ...
   * 정책과 관련된 파일 목록 수집
   */
  private getFilesForPolicy(codeIndex: CodeIndex, policy: PolicyInfo): string[] {
   ...
   * 기획자 체크리스트 생성
   */
  private generateChecklist(matchedPolicies: PolicyInfo[], _changeDesc: string): string[] {
