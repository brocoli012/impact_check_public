...
import type { GapCheckResult, GapItem } from '../core/cross-project/types';
...
export class GapCheckCommand implements Command {
  ...
  async execute(): Promise<CommandResult> {
    try {
      ...
      if (isFix) {
        ...
        return {
          code: fixResult.failed > 0 ? ResultCode.PARTIAL : ResultCode.SUCCESS,
   ...
   * 갭 탐지 결과를 테이블 형태로 출력
   */
  private printResult(result: GapCheckResult): void {
    ...
    if (gaps.length === 0) {
      ...
      if (excludedCounts) {
        ...
        if (completed > 0 || onHold > 0 || archived > 0) {
          console.log(
            `  Excluded by status: ${completed} completed, ${onHold} on-hold, ${archived} archived\n`,
          );
        }
      }
      return;
    ...
    logger.header('Gap Check Results');
    console.log('');

    // 테이블 헤더
    console.log(
      `  ${'SEVERITY'.padEnd(10)} ${'TYPE'.padEnd(22)} ${'PROJECT'.padEnd(20)} ${'FIXABLE'.padEnd(8)} DESCRIPTION`,
    );
    ...
    for (const gap of sorted) {
      console.log(
        `  ${gap.severity.padEnd(10)} ${gap.type.padEnd(22)} ${gap.projectId.padEnd(20)} ${(gap.fixable ? 'yes' : 'no').padEnd(8)} ${gap.description}`,
      );
    }
    ...
    console.log('');
    console.log(
      `  Total: ${summary.total} gaps (${summary.high} high, ${summary.medium} medium, ${summary.low} low), ${summary.fixable} fixable`,
    );
   ...
   * 수정 결과 출력
   */
  private printFixResult(fixResult: { fixed: number; failed: number; details: Array<{ gap: GapItem; success: boolean; message: string }> }): void {
