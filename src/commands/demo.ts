...
/** 데모용 샘플 분석 결과 */
function createDemoResult(): ConfidenceEnrichedResult {
...
export class DemoCommand implements Command {
  ...
  async execute(): Promise<CommandResult> {
    ...
    try {
      logger.header('Impact Checker - Demo');
      console.log('');
      ...
      const demoDir = getProjectDir(demoProjectId);
      ensureDir(demoDir);
      ...
      logger.error(`데모 실행 실패: ${errorMsg}`);
      return {
