...
interface ParserCache {
  treeSitterModule: any;
  javaParser: any;
  kotlinParser: any;
  loadResult: any; // undefined=미시도, null=실패, 그 외=성공
}
 ...
 */
function getParserCache(): ParserCache {
 ...
 */
function loadTreeSitterSync(): any {
  const cache = getParserCache();
  if (cache.loadResult !== undefined) return cache.loadResult;
 ...
 */
function nativeLoad(moduleName: string): any {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const Module = require('module') as any;
...
export async function getJavaParser(): Promise<TreeSitterParser | null> {
  const cache = getParserCache();
  if (cache.javaParser) return cache.javaParser;
...
export async function getKotlinParser(): Promise<TreeSitterParser | null> {
  const cache = getParserCache();
  if (cache.kotlinParser) return cache.kotlinParser;
