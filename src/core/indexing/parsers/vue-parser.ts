...
export class VueParser extends BaseParser {
  ...
  async parse(filePath: string, content: string): Promise<ParsedFile> {
    ...
    try {
      ...
      logger.debug(`Failed to parse Vue SFC ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
    }
   ...
   * <script> 블록 추출
   */
  private extractScriptBlocks(content: string): { content: string; lang: string; isSetup: boolean }[] {
    const blocks: { content: string; lang: string; isSetup: boolean }[] = [];
   ...
   */
  private detectVuePatterns(scriptContent: string, _filePath: string, result: ParsedFile): void {
   ...
   */
  private detectTemplateComponents(content: string, filePath: string, result: ParsedFile): void {
    ...
    while ((tagMatch = CUSTOM_COMPONENT_RE.exec(templateContent)) !== null) {
      ...
      // HTML/SVG 태그 제외
      if (HTML_TAGS.has(tagName.toLowerCase()) || SVG_TAGS.has(tagName)) continue;

      // Vue 내장 컴포넌트 제외
      if (['component', 'transition', 'keep-alive', 'teleport', 'suspense', 'slot'].includes(tagName.toLowerCase())) continue;
    ...
    for (const name of componentNames) {
      ...
      if (!existing) {
        ...
        const hasImport = result.imports.some(imp =>
          imp.specifiers.includes(pascalName) || imp.specifiers.includes(name)
        );
