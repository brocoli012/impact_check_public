...
export class TypeScriptParser extends BaseParser {
  ...
  private detectTypeOrmEntity(
    ...
    if (!decorators || !Array.isArray(decorators)) return;

    // @Entity() 데코레이터 찾기
    let entityDecorator: any = null;
    ...
    for (const member of node.body) {
      if (member.type === 'ClassProperty' || member.type === 'ClassMethod') {
        const memberDecorators = (member as any).decorators;
        if (!memberDecorators || !Array.isArray(memberDecorators)) continue;

        const memberDecoNames: string[] = [];
        for (const dec of memberDecorators) {
          ...
          if (decName) memberDecoNames.push(decName);
        }
        ...
        if (member.type === 'ClassProperty') {
          ...
          if (keyNode?.type === 'Identifier') {
            ...
            const typeAnno = (member as any).typeAnnotation;
            ...
            const isRelation = memberDecoNames.some(d =>
              ['ManyToOne', 'OneToMany', 'ManyToMany', 'OneToOne'].includes(d)
            );
            const relAnnotation = memberDecoNames.find(d =>
              ['ManyToOne', 'OneToMany', 'ManyToMany', 'OneToOne'].includes(d)
            );
