 ...
 */
export class SharedEntityIndexer {
  /**
   * 공유 엔티티 역인덱스 빌드
   *
  ...
  build(projectIndexMap: Map<string, CodeIndex>): SharedEntityIndex {
    const tables: Record<string, TableReference[]> = {};
    const events: Record<string, EventReference[]> = {};
    ...
    const result: SharedEntityIndex = {
      version: 1,
    ...
    logger.info(
      `SharedEntityIndex: ${tableCount} tables (${sharedTableCount} shared), ` +
  ...
  getSharedTables(index: SharedEntityIndex): Record<string, TableReference[]> {
    const shared: Record<string, TableReference[]> = {};
  ...
  getSharedEvents(index: SharedEntityIndex): Record<string, EventReference[]> {
    const shared: Record<string, EventReference[]> = {};
  ...
  private buildTableIndex(
    ...
    if (!index.models || index.models.length === 0) return;
    ...
    for (const model of index.models) {
      ...
      const accessPattern: TableReference['accessPattern'] = 'read-write';
  ...
  private buildEventIndex(
    ...
    if (!index.events || index.events.length === 0) return;
