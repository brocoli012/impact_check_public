 ...
 */
export class ApiContractChecker {
  /**
   * 두 프로젝트의 API 계약 비교
   *
  ...
  async checkContracts(
    ...
    previousProviderApis?: ApiEndpoint[],
  ): Promise<ApiContractChange[]> {
    if (!previousProviderApis) {
      logger.debug('이전 Provider API 없음, 변경 감지 불가');
      return [];
    }
    ...
    const previousApiMap = new Map<string, ApiEndpoint>();
    for (const api of previousProviderApis) {
      previousApiMap.set(api.path, api);
    }

    // 현재 API를 path 기준으로 Map 구축
    const currentApiMap = new Map<string, ApiEndpoint>();
    ...
    logger.info(`API 계약 변경 감지: ${changes.length}건`);
    return changes;
  }
  ...
  private detectChange(
    ...
    const hasParamsChange =
      JSON.stringify(current.requestParams.sort()) !==
