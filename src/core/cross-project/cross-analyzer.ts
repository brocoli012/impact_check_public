...
export class CrossAnalyzer {
  constructor(
    private manager: CrossProjectManager,
    private contractChecker: ApiContractChecker,
  ) {}
  ...
  async analyze(
    ...
    logger.info(`크로스 프로젝트 분석 시작: ${sourceProjectId}`);

    // 1. 연결된 프로젝트 조회
    let links = await this.manager.getLinks(sourceProjectId);

    if (links.length === 0) {
      logger.info('연결된 프로젝트가 없습니다.');
      ...
      if (group) {
        ...
        links = links.filter((l) => {
          const otherProjectId =
            l.source === sourceProjectId ? l.target : l.source;
          return groupProjectIds.has(otherProjectId);
        });
      } else {
        logger.warn(`그룹을 찾을 수 없습니다: ${options.groupName}`);
        return { affectedProjects: [], apiContractChanges: [] };
      }
    }
    ...
    if (!sourceIndex) {
      logger.warn(`소스 프로젝트 인덱스를 로드할 수 없습니다: ${sourceProjectId}`);
      return { affectedProjects: [], apiContractChanges: [] };
    }
    ...
    const allContractChanges: ApiContractChange[] = [];
    ...
    for (const targetId of targetProjectIds) {
      try {
        ...
        if (!targetIndex) {
          logger.debug(`대상 프로젝트 인덱스 로드 실패, 건너뜁니다: ${targetId}`);
          continue;
        }
        ...
        // 4b. API 계약 변경 감지
        const contractChanges = await this.contractChecker.checkContracts(
          sourceApis,
          targetApis,
          sourceApis, // previousProviderApis: 현재 소스 API를 기준으로 대상과 비교
        );
        ...
        // 4e. 영향을 받는 컴포넌트 수 계산
        const affectedComponents = this.countAffectedComponents(
          targetIndex,
          affectedApiPaths,
        );
        ...
        logger.debug(
          `프로젝트 분석 중 오류: ${targetId}: ${err instanceof Error ? err.message : String(err)}`,
        );
        continue;
      }
    }
    ...
    logger.info(
      `크로스 프로젝트 분석 완료: ${affectedProjects.length}개 프로젝트 영향`,
    );
  ...
  private findAffectedApis(
    sourceApis: ApiEndpoint[],
    targetApis: ApiEndpoint[],
  ): string[] {
  ...
  private calculateImpactLevel(
    ...
    const hasCritical = contractChanges.some(
      (c) =>
        c.changeType === 'remove' && c.consumers.length > 0,
    );
    if (hasCritical) return 'critical';

    // high 체크: 비하위 호환 변경 (modify with warning+ severity)
    const hasHigh = contractChanges.some(
      (c) => c.changeType === 'modify' || c.severity === 'warning',
    );
    if (hasHigh) return 'high';

    // medium 체크: 하위 호환 변경 (add)
    const hasMedium = contractChanges.some(
      (c) => c.changeType === 'add',
    );
    if (hasMedium) return 'medium';

    // low: 간접 영향 (공통 API 존재)
    if (affectedApiPaths.length > 0) return 'low';
  ...
  private countAffectedComponents(
    targetIndex: { components: Array<{ apiCalls: string[] }> },
    affectedApiPaths: string[],
  ): number {
    if (affectedApiPaths.length === 0) return 0;
    ...
    for (const comp of targetIndex.components) {
      ...
      const hasAffectedApi = comp.apiCalls.some((apiId) =>
        affectedPathSet.has(apiId),
      );
      if (hasAffectedApi) {
        count++;
      }
