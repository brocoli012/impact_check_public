...
interface CacheEntry {
  data: Map<string, AnnotationFile>;
  timestamp: number;
...
export class AnnotationManager {
  ...
  private readonly loadAllCache = new Map<string, CacheEntry>();
    ...
    if (cached) {
      ...
      if (!isExpired) {
        ...
        if (currentLastUpdated === cached.lastUpdatedAt) {
          logger.debug(`loadAll cache hit for project: ${projectId}`);
          return cached.data;
        }
        logger.debug(`loadAll cache invalidated (meta changed) for project: ${projectId}`);
      } else {
        logger.debug(`loadAll cache expired for project: ${projectId}`);
      }
    }
