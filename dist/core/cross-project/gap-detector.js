...
const cross_project_manager_1 = require("./cross-project-manager");
...
class GapDetector {
    ...
    constructor(basePath) {
        this.impactDir = (0, file_1.getImpactDir)(basePath);
        this.crossManager = new cross_project_manager_1.CrossProjectManager(this.impactDir);
    ...
    async fix(gaps) {
        ...
        if (configModified) {
            ...
            (0, file_1.writeJsonFile)(configPath, config);
        }
     ...
     * (프로젝트가 등록되지 않은 경우)
     */
    isDeletedProjectLink(gap, config) {
        const projects = this.loadProjects();
     ...
     * - 프로젝트가 삭제/미존재하는 링크
     */
    detectStaleLinks(config, projects) {
        const gaps = [];
        const projectIds = new Set(projects.map(p => p.id));
        for (const link of config.links) {
            ...
            if (link.confirmedAt) {
                ...
                const latestUpdatedAt = Math.max(sourceIndexUpdatedAt ? new Date(sourceIndexUpdatedAt).getTime() : 0, targetIndexUpdatedAt ? new Date(targetIndexUpdatedAt).getTime() : 0);
     ...
     */
    detectUnanalyzedProjects(config, projects) {
        const gaps = [];
     ...
     */
    async detectLowConfidence(projects) {
        const gaps = [];
     ...
     */
    getLastGitCommitDate(projectPath) {
        try {
            const result = (0, child_process_1.execSync)('git log -1 --format=%ci', {
                cwd: projectPath,
                timeout: 2000,
                encoding: 'utf-8',
