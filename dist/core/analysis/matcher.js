...
class IndexMatcher {
     ...
     */
    match(spec, index) {
        logger_1.logger.info('Starting index matching...');
        // 검색 키워드 수집
        const searchKeywords = this.collectKeywords(spec);
        logger_1.logger.debug(`Search keywords: ${searchKeywords.join(', ')}`);
        // 각 엔티티 타입별 매칭
        ...
        // 초기 매칭 결과
        let matched = { screens, components, apis, models };
        // 의존 그래프 기반 확장 (1-hop)
        matched = this.expandByDependency(matched, index.dependencies);
        logger_1.logger.info(`Matched: ${matched.screens.length} screens, ` +
     ...
     * 컴포넌트 매칭
     */
    matchComponents(keywords, components) {
        const matched = [];
     ...
     */
    calculateSimilarity(keyword, target) {
        ...
        if (tgt.includes(kw))
            return 0.8;
        if (kw.includes(tgt) && tgt.length >= 2)
            return 0.6;
        // 경로/이름 부분 매칭
     ...
     * @returns 확장된 매칭 결과
     */
    expandByDependency(matched, graph) {
