...
class PolicyMatcher {
     ...
     * @returns 정책 경고 목록
     */
    match(impact, policies) {
        logger_1.logger.info('Matching policies...');
        const warnings = [];
        let counter = 0;
        // 영향 받는 파일 목록 수집
        ...
        for (const policy of policies) {
            ...
            // 관련 컴포넌트/API 기반 매칭
            const componentMatch = policy.relatedComponents.some(c => this.isRelatedToTasks(c, impact));
            const apiMatch = policy.relatedApis.some(a => affectedApis.has(a));
            // 모듈명 기반 매칭
            const moduleMatch = policy.relatedModules.some(m => this.isModuleAffected(m, affectedFiles));
            if (fileMatch || componentMatch || apiMatch || moduleMatch) {
                counter++;
                // 관련 작업 ID 수집
                const relatedTaskIds = impact.tasks
                    .filter(t => t.affectedFiles.some(f => f === policy.filePath) ||
        ...
        logger_1.logger.info(`Matched ${warnings.length} policy warnings.`);
        return warnings;
    }
    /**
     * 컴포넌트가 작업과 관련이 있는지 확인
     */
    isRelatedToTasks(componentId, impact) {
        return impact.tasks.some(t => t.affectedFiles.some(f => f.includes(componentId)));
    }
    /**
     * 모듈이 영향 받는 파일에 포함되는지 확인
     */
    isModuleAffected(moduleName, affectedFiles) {
        const normalizedModule = moduleName.toLowerCase();
        return Array.from(affectedFiles).some(f => f.toLowerCase().includes(normalizedModule));
    }
    /**
     * 경고 심각도 결정
     */
    determineSeverity(policy, isDirectFileMatch, impact) {
     ...
     * 경고 메시지 생성
     */
    buildWarningMessage(policy, fileMatch, componentMatch, apiMatch) {
        const reasons = [];
        if (fileMatch) {
...
exports.PolicyMatcher = PolicyMatcher;
