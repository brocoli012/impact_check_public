...
class OwnerMapper {
     ...
     */
    map(impact, owners) {
        logger_1.logger.info('Mapping owners...');
        const notifications = [];
        let counter = 0;
        // 영향 받는 파일 목록 수집
        ...
        for (const system of owners.systems) {
            ...
            if (matchedPaths.length > 0) {
                ...
                const relatedTaskIds = impact.tasks
                    .filter(t => t.affectedFiles.some(f => system.relatedPaths.some(p => f.includes(p))))
                    .map(t => t.id);
                // 이메일 초안 생성
        ...
        logger_1.logger.info(`Mapped ${notifications.length} owner notifications.`);
        return notifications;
    }
    /**
     * 시스템의 관련 경로 중 영향 받는 파일과 매칭되는 경로 탐색
     */
    findMatchingPaths(system, affectedFiles) {
        const matched = [];
     ...
     * 확인 요청 이메일 초안 생성
     */
    generateEmailDraft(system, impact, relatedTaskIds) {
...
exports.OwnerMapper = OwnerMapper;
//# sourceMappingURL=owner-mapper.js.map