...
const ANNOTATION_POLICY_PATTERNS = [
    {
...
class AnnotationGenerator {
     ...
     */
    async generateForFile(filePath, parsedFile, projectPath) {
        const fileContext = this.buildFileContext(filePath, parsedFile);
        const system = this.inferSystem(filePath, projectPath);
        const annotations = parsedFile.functions.map((func) => this.analyzeFunction(func, filePath, fileContext));
        ...
        // 파일 요약 생성
        const fileSummary = this.generateFileSummary(filePath, parsedFile, annotations, fileContext);
     ...
     */
    async generateBatch(files, projectPath, onProgress) {
        ...
        for (let i = 0; i < filteredFiles.length; i++) {
            ...
            const annotationFile = await this.generateForFile(filePath, parsedFile, projectPath);
     ...
     */
    analyzeFunction(func, _filePath, fileContext) {
     ...
     * 함수에서 정책 추론
     */
    inferPolicies(func, _fileContext) {
        ...
        for (const policyPattern of POLICY_PATTERNS) {
            ...
            if (!matched)
                continue;
            const inputVariables = func.params.map((p) => ({
                name: p.name,
                type: p.type ?? 'unknown',
                description: `${p.name} 파라미터`,
            }));
            const outputVariables = func.returnType
                ? [
                    {
                        name: 'result',
     ...
     */
    classifyFunctionType(func, fileContext) {
     ...
     */
    generateEnrichedComment(func, fileContext) {
        const funcName = this.getBaseFunctionName(func.name);
        const parts = [];
        // 함수 의도 추론
     ...
     * 최대 1.0
     */
    calculateConfidence(func, policies, originalComment) {
        let confidence = 0.5;
        // 기존 주석(JSDoc) 있으면 +0.1
        if (originalComment) {
            confidence += 0.1;
     ...
     * FileContext 생성
     */
    buildFileContext(filePath, parsedFile) {
     ...
     * 시스템 이름 추론 (프로젝트 구조에서)
     */
    inferSystem(filePath, _projectPath) {
     ...
     * 추론 근거 문자열 생성
     */
    buildInferredFrom(func, fileContext) {
        const sources = [];
     ...
     */
    calculatePolicyConfidence(func) {
        let conf = 0.4;
        if (func.params.some((p) => p.type !== undefined))
            conf += 0.1;
        if (func.returnType)
            conf += 0.1;
        const bodyLines = func.endLine - func.startLine + 1;
     ...
     */
    calculateAnnotationPolicyConfidence(annotation) {
        let conf = 0.7;
        // 어노테이션에 파라미터가 있으면 더 구체적이므로 +0.1
        if (annotation.includes('('))
            conf += 0.1;
        // 어노테이션에 value나 key 같은 상세 설정이 있으면 +0.1
        if (/value\s*=|key\s*=|cron\s*=|maxAttempts/i.test(annotation))
            conf += 0.1;
        return Math.min(conf, 1.0);
    }
    /**
     * 파일 요약 생성
     */
    generateFileSummary(_filePath, parsedFile, annotations, fileContext) {
     ...
     * 비즈니스 도메인 추론
     */
    inferBusinessDomain(parsedFile, fileContext) {
        ...
        if (/order|cart|checkout|purchase/.test(combined))
            return '주문';
        if (/product|item|catalog|goods/.test(combined))
            return '상품';
        if (/user|member|profile|account/.test(combined))
            return '회원';
        if (/payment|billing|invoice/.test(combined))
            return '결제';
        if (/ship|deliver|logistics/.test(combined))
            return '배송';
        if (/auth|login|token|session/.test(combined))
            return '인증';
        if (/discount|coupon|promotion/.test(combined))
            return '프로모션';
        if (/review|rating|comment/.test(combined))
            return '리뷰';
        if (/notification|alert|message/.test(combined))
            return '알림';
        if (/search|filter|sort/.test(combined))
            return '검색';
        return '일반';
    }
    /**
     * 파일에서 키워드 추출
     */
    extractKeywords(parsedFile, fileContext) {
