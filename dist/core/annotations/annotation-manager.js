...
/** 기본 보강 주석 저장 경로 */
const DEFAULT_BASE_PATH = path.join(process.env.HOME || process.env.USERPROFILE || '~', '.impact');
/** 보강 주석 파일 확장자 */
...
class AnnotationManager {
     ...
     */
    async save(projectId, filePath, annotation) {
        ...
        logger_1.logger.debug(`Annotation saved: ${annotationPath}`);
    }
    /**
     * 보강 주석 파일을 YAML에서 로드
     * @param projectId - 프로젝트 ID
    ...
    async load(projectId, filePath) {
        ...
        try {
            ...
            logger_1.logger.warn(`Failed to load annotation: ${annotationPath}`, err);
            return null;
        }
    }
    /**
     ...
     */
    async isChanged(projectId, filePath, currentHash) {
    ...
    async merge(existing, updated) {
        ...
        const mergedAnnotations = updated.annotations.map((updatedAnn) => {
     ...
     */
    async cleanup(projectId, filePath, currentFunctions) {
        const existing = await this.load(projectId, filePath);
        if (!existing) {
            return;
        }
        const currentSet = new Set(currentFunctions);
        const cleanedAnnotations = existing.annotations.filter((ann) => currentSet.has(ann.function));
        if (cleanedAnnotations.length !== existing.annotations.length) {
            ...
            logger_1.logger.info(`Cleaned up ${removed} annotations for removed functions in ${filePath}`);
            const updatedFile = {
                ...existing,
    ...
    async updateMeta(projectId) {
        ...
        logger_1.logger.debug(`Meta updated: ${metaPath}`);
        return meta;
    }
    /**
     * meta.json 읽기
    ...
    async getMeta(projectId) {
        ...
        try {
            ...
            logger_1.logger.warn(`Failed to read meta.json: ${metaPath}`, err);
            return null;
        }
    }
    /**
    ...
    async delete(projectId, filePath) {
        ...
        if (fs.existsSync(annotationPath)) {
            ...
            logger_1.logger.debug(`Annotation deleted: ${annotationPath}`);
        }
    }
    /**
     * 디렉토리 존재 확인/생성
     ...
     */
    getAnnotationPath(projectId, filePath) {
        // 선행 슬래시 제거 (절대경로에서 상대경로로 변환)
        const normalizedPath = filePath.replace(/^\/+/, '');
        return path.join(this.basePath, 'annotations', projectId, `${normalizedPath}${ANNOTATION_EXTENSION}`);
    }
    /**
     * meta.json 파일 경로 반환
     * @param projectId - 프로젝트 ID
