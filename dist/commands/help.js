...
const COMMAND_HELP = {
    ...
    projects: {
        usage: '/impact projects [--switch <name>] [--remove <name>] [--info <name>] [--link ...] [--unlink ...] [--links] [--detect-links] [--group ...] [--groups]',
        description: '멀티 프로젝트를 관리합니다.',
        options: [
            '--switch <name>  활성 프로젝트 전환',
            '--remove <name>  프로젝트 등록 해제',
            '--info <name>    프로젝트 상세 조회',
            '--link <source> <target> --type <type>  크로스 프로젝트 의존성 등록',
            '--unlink <source> <target>              크로스 프로젝트 의존성 해제',
            '--links [projectId]                     의존성 목록 조회 (프로젝트별 필터 가능)',
            '--detect-links                          API 경로 기반 자동 의존성 감지',
            '--group <name> --add <project-id>       그룹에 프로젝트 추가',
            '--group <name> --remove <project-id>    그룹에서 프로젝트 제거',
            '--groups                                전체 그룹 목록 조회',
        ],
        examples: [
            '/impact projects',
            ...
            '/impact projects --link frontend-app backend-api --type api-consumer',
            '/impact projects --unlink frontend-app backend-api',
            '/impact projects --links',
            '/impact projects --links frontend-app',
            '/impact projects --detect-links',
            '/impact projects --group commerce --add frontend-app',
            '/impact projects --group commerce --remove frontend-app',
            '/impact projects --groups',
        ],
    },
    demo: {
    ...
    'cross-analyze': {
        usage: '/impact cross-analyze [--source <project-id>] [--group <group-name>]',
        description: '크로스 프로젝트 영향도 분석을 수행합니다.',
        options: [
        ...
        examples: [
            ...
            '/impact cross-analyze --source backend-api',
            '/impact cross-analyze --group commerce',
...
class HelpCommand {
    ...
    async execute() {
        ...
        if (targetCommand && COMMAND_HELP[targetCommand]) {
            ...
            logger_1.logger.warn(`'${targetCommand}' 명령어를 찾을 수 없습니다.`);
     ...
     */
    showAllCommands() {
        logger_1.logger.header('Kurly Impact Checker v1.0.0');
        console.log('\n  기획서 기반 코드 영향도 분석 도구\n');
    ...
    showCommandHelp(command) {
        ...
        if (!info)
            return;
        logger_1.logger.header(`/impact ${command}`);
        console.log(`\n  ${info.description}\n`);
