...
exports.LLMRouter = exports.ProviderRegistry = exports.NoProviderConfiguredError = exports.ProviderNotFoundError = void 0;
...
class ProviderRegistry {
    ...
    register(provider) {
        ...
        logger_1.logger.debug(`LLM provider registered: ${provider.name}`);
    }
    /**
     * 프로바이더 조회
     * @param name - 프로바이더 이름
...
class LLMRouter {
    ...
    route(task) {
        ...
        logger_1.logger.debug(`Provider '${providerName}' not available for task '${task}'. Using fallback.`);
     ...
     */
    setRoute(task, providerName) {
        this.routingTable[task] = providerName;
        logger_1.logger.debug(`Route updated: ${task} -> ${providerName}`);
    }
    /**
     * 현재 라우팅 테이블을 반환
     * @returns 라우팅 테이블 복사본
     ...
     * @returns LLM 응답
     */
    async chatWithRetry(task, messages, options, retryOpts) {
        const provider = this.route(task);
        return (0, retry_1.retryWithBackoff)(() => provider.chat(messages, options), {
            maxRetries: 3,
            initialDelayMs: 1000,
            backoffMultiplier: 2,
            ...retryOpts,
